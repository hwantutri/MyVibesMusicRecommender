...
switch ($action) {
   case "Import":
     $dbname = "myvibes"; //database name
$dbconn = pg_pconnect("host=localhost port=5432 dbname=$dbname user=myvibes password=tseree"); //connectionstring
     if (!$dbconn) {
       echo "Can't connect.\n";
       exit;
     }
...
$dbconn = pg_pconnect("host=localhost port=5432 dbname=$dbname user=myvibes password=tseree"); //connectionstring
 ...
 $back = fopen("/var/www/html/myvibes/here/$dbname.sql","w");
...
AND pg_type.oid=atttypid AND attnum>0 AND atthasdef='f' AND lower(relname)='$table' "); while($r = pg_fetch_row($res2))
   {
   $str .= "\n" . $r[1]. " " . $r[2];
    if ($r[2]=="varchar")
   {
...
JOIN pg_catalog.pg_class clf ON (ct.confrelid = clf.oid AND clf.relkind = 'r')
 ...
 while($row = pg_fetch_row($res))
 {
   ...
   $str .= "\n--\n\n";
$str .= "ALTER TABLE ONLY ".$row[0] . " ADD CONSTRAINT " . $row[1] . " " . $row[2] . ";"; } fwrite($back,$str);
 fclose($back);
 dl_file("$dbname.sql");
 break;
}
